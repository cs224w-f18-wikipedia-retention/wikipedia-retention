...
Xnorm_b = X[:,0:36]
clf_b = LR(penalty='l2', class_weight='balanced').fit(Xnorm_b,y)
preds_b = clf_b.predict_proba(Xnorm_b)[:,1]
ll_b = log_loss(y, preds_b, s_weights)

# logistic regression with base+roles only
Xnorm_r = np.delete(Xnorm,[36,37,38],1)
clf_r = LR(penalty='l2', class_weight='balanced').fit(Xnorm_r,y)
preds_r = clf_r.predict_proba(Xnorm_r)[:,1]
ll_r = log_loss(y, preds_r, s_weights)
